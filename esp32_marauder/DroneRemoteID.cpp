...
constexpr int16_t ROW_START_Y = 42;
...
void drawHeader() {
  MiniV3Ui::header(display_obj.tft, "DRONE REMOTE ID",
                   String("CH ") + scanChannel + "  PASSIVE", TFT_ORANGE);
}
...
void drawList(uint8_t selected) {
  ...
  display_obj.tft.setCursor(4, 33);
  display_obj.tft.printf("Found %u", count);
  ...
  if (count == 0)
    MiniV3Ui::emptyState(display_obj.tft, "Listening for broadcasts",
                         "WiFi NAN/Beacon + BLE");
  MiniV3Ui::footer(display_obj.tft, "UP/DN SELECT  R DETAIL  C EXIT");
}
...
void drawDetail(uint8_t selected, uint8_t page) {
  ...
  if (page == 0) {
    ...
    display_obj.tft.drawString(drone.id, 2, 34, 1);
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    display_obj.tft.drawString(drone.address, 2, 48, 1);
    ...
    display_obj.tft.drawString(String(drone.rssi) + "dBm " +
                               methodName(drone.methods), 2, 90, 1);
    display_obj.tft.drawString(String("Seen ") +
        ((millis() - drone.lastSeen) / 1000) + "s ago", 2, 104, 1);
  }
  else if (page == 1) {
    display_obj.tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    display_obj.tft.drawString("Aircraft location", 2, 34, 1);
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    if (drone.messagesSeen & (1 << LOCATION)) {
      display_obj.tft.drawString("Lat: " + String(drone.latitude, 6), 2, 48, 1);
      display_obj.tft.drawString("Lon: " + String(drone.longitude, 6), 2, 62, 1);
      ...
      display_obj.tft.drawString("Not observed", 2, 52, 1);
    }
  }
  else if (page == 2) {
    display_obj.tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    display_obj.tft.drawString("Operator", 2, 34, 1);
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    display_obj.tft.setViewport(2, 48, TFT_WIDTH - 4, 13);
    display_obj.tft.setCursor(0, 2);
    ...
    if (drone.messagesSeen & (1 << SYSTEM)) {
      ...
      display_obj.tft.drawString("Location not observed", 2, 68, 1);
    }
  }
  else {
    display_obj.tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    display_obj.tft.drawString("Messages / description", 2, 34, 1);
    display_obj.tft.setTextColor(TFT_WHITE, TFT_BLACK);
    ...
    snprintf(messageFlags, sizeof(messageFlags), "B%c L%c A%c S%c Y%c O%c",
             ...
             (drone.messagesSeen & (1 << SYSTEM)) ? '+' : '-',
             (drone.messagesSeen & (1 << OPERATOR_ID)) ? '+' : '-');
    display_obj.tft.drawString(messageFlags, 2, 50, 1);
    display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
    display_obj.tft.setTextWrap(true);
    display_obj.tft.setCursor(2, 66);
    display_obj.tft.print(drone.description[0] ? drone.description :
                          "Description not observed");
    display_obj.tft.setTextWrap(false);
  }

  MiniV3Ui::footer(display_obj.tft, "L LIST  R NEXT  C EXIT");
}
...
void showError(const char* message) {
  ...
  display_obj.tft.setTextWrap(false);
  MiniV3Ui::footer(display_obj.tft, "CENTER EXIT", TFT_WHITE);
}
